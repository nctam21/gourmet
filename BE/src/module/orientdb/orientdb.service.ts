...
export class OrientDbService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        '[OrientDbService] Binary connection disabled or failed:',
  ...
  getDb(): ODatabaseSession {
    if (!this.binaryEnabled || !this.db) {
      throw new Error(
        'OrientDB binary driver is disabled or not connected. Enable by setting ORIENTDB_BINARY_ENABLED=true',
      );
    }
    return this.db;
  }
