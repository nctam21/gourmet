...
export class FoodController {
    ...
    async getTop6MostViewedFoods(): Promise<{
        ...
        try {
            ...
            const results = await this.foodService['orientDbHttpService'].queryAll<any>(query, 6);
    ...
    async testDatabaseConsistency(
        ...
        try {
            ...
            if (regionResult.success && regionResult.result && regionResult.result.length > 0) {
                const region = regionResult.result[0];
                const inFromRegion = region.in_FROM_REGION || [];

                console.log(`[Controller] Checking ${inFromRegion.length} RIDs from in_FROM_REGION`);

                // Kiểm tra từng RID
                for (const foodRid of inFromRegion) {
                    const foodCheckQuery = `SELECT @rid, name FROM Food WHERE @rid = '${foodRid}' LIMIT 1`;
                    const foodCheckResult = await this.foodService.testDirectQuery(foodCheckQuery);

                    if (foodCheckResult.success && foodCheckResult.result && foodCheckResult.result.length > 0) {
            ...
            const allFoodsResult = await this.foodService.testDirectQuery(allFoodsQuery);

            // Tìm foods có out_FROM_REGION chứa region RID
            let foodsWithRegion: any[] = [];
            if (allFoodsResult.success && allFoodsResult.result) {
                foodsWithRegion = allFoodsResult.result.filter((food: any) =>
            ...
            return {
                ...
                foodsWithRegion,
                summary: {
                    totalRidsInRegion: regionResult.result?.[0]?.in_FROM_REGION?.length || 0,
    ...
    async testFoodQuery(
        @Param('rids') rids: string
    ): Promise<any> {
        try {
            // Test query Food với RIDs cụ thể
            const foodRids = rids.split(',').map(rid => `'${rid}'`).join(',');
    ...
    async testExactRelationship(
        ...
        try {
            ...
            const foodSampleResult = await this.foodService.testDirectQuery(foodSampleQuery);
            ...
            const directFoodsQuery = `
                ...
                LIMIT 5
            `;
            const directFoodsResult = await this.foodService.testDirectQuery(directFoodsQuery);
            ...
            const reverseQuery = `
                ...
                LIMIT 5
            `;
            const reverseResult = await this.foodService.testDirectQuery(reverseQuery);
    ...
    async testDeleteOperation(@Param('rid') rid: string): Promise<{
        ...
        try {
            ...
            const testResult = await this.foodService['orientDbHttpService'].queryOne<any>(testDeleteSql);
